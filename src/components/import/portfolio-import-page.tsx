...
const ACKNOWLEDGEMENT_TEXT =
  "I reviewed the valid rows and understand that this creates new grant records.";
...
function stateLabel(row: PortfolioImportPreviewRow): string {
  if (row.state === "collapsed_duplicate") return "Duplicate";
...
function CountSummary({ label, value, tone = "default" }: { label: string; value: number; tone?: "default" | "muted" | "danger" }): React.ReactNode {
  const valueClass = tone === "danger" ? "text-destructive" : tone === "muted" ? "text-muted-foreground" : "text-foreground";
  return (
    <div className="min-w-0">
      <dt className="text-label text-muted-foreground">{label}</dt>
      <dd className={`mt-0.5 text-metric ${valueClass}`}>{value}</dd>
    </div>
  );
}
...
function ImportRow({ row }: { row: PortfolioImportPreviewRow }): React.ReactNode {
  ...
  return (
    <li>
      <article
        className={`rounded-md border p-4 ${row.state === "invalid" ? "border-destructive/40" : "border-border"}`}
      ...
      >
        <header className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
          <div className="min-w-0">
            ...
            <h4 id={`import-row-${row.sourceRowNumber}`} className="mt-1 break-words text-base font-semibold text-foreground">
              {rowTitle}
            </h4>
          </div>
        ...
        {row.collapsedIntoSourceRow !== null && (
          <p className="mt-4 border-t border-border pt-3 text-sm text-muted-foreground">
            Duplicate of source row {row.collapsedIntoSourceRow}; no new grant will be created.
          </p>
        )}

        {row.collapsedSourceRows.length > 0 && (
          <p className="mt-4 border-t border-border pt-3 text-sm text-muted-foreground">
            Duplicate source rows: {row.collapsedSourceRows.join(", ")}.
          </p>
        )}
...
function CompletionState({ result, onStartOver }: { result: PortfolioImportCommitResult; onStartOver: () => void }): React.ReactNode {
  const { counts } = result;
  return (
    <section className="rounded-lg border border-border bg-card p-5 sm:p-6" aria-labelledby="import-complete-title">
      <h1 id="import-complete-title" className="text-title text-foreground">Import complete</h1>
      <p className="mt-2 max-w-2xl text-base leading-6 text-muted-foreground">
        Created the valid grants from the {result.worksheet} worksheet. The original workbook and preview were not retained.
      </p>

      <dl className="mt-6 grid gap-x-6 gap-y-3 border-y border-border py-4 sm:grid-cols-2 lg:grid-cols-3" aria-label="Import completion counts">
        <CountSummary label="Funders created" value={counts.createdFunders} />
        <CountSummary label="Funders reused" value={counts.reusedFunders} />
        <CountSummary label="Grants created" value={counts.createdGrants} />
        <CountSummary label="Duplicates collapsed" value={counts.collapsed} tone="muted" />
        <CountSummary label="Rows excluded" value={counts.excluded} tone={counts.excluded > 0 ? "danger" : "muted"} />
        <CountSummary label="Rows skipped" value={counts.structural} tone="muted" />
      </dl>
    ...
    <section className="mt-8 flex flex-col gap-6" aria-labelledby="import-preview-title">
      <div className="rounded-lg border border-border bg-card p-5 sm:p-6">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
          <div className="min-w-0">
            <h2 id="import-preview-title" className="text-h2 text-foreground">Review import</h2>
            <p className="mt-1 break-words text-sm text-muted-foreground">
              File: {preview.fileName}; sheet: {preview.worksheet}; header row: {preview.headerRowNumber}
            </p>
          </div>
          <p className="text-sm text-muted-foreground">Workbook size: {formatFileSize(file.size)}</p>
        </div>

        <dl className="mt-6 grid gap-x-5 gap-y-3 border-y border-border py-4 sm:grid-cols-2 lg:grid-cols-5" aria-label="Import preview counts">
          <CountSummary label="Structural rows skipped" value={preview.counts.structural} tone="muted" />
          <CountSummary label="Candidate rows" value={preview.counts.candidate} />
          <CountSummary label="Valid rows" value={preview.counts.valid} />
          <CountSummary label="Invalid / excluded" value={preview.counts.invalid} tone={preview.counts.invalid > 0 ? "danger" : "muted"} />
          <CountSummary label="Collapsed duplicate rows" value={preview.counts.collapsed} tone="muted" />
        </dl>

        <div className="mt-6 grid gap-6 border-t border-border pt-6 lg:grid-cols-2 lg:gap-8">
          <section className="min-w-0" aria-labelledby="import-headers-title">
            <h3 id="import-headers-title" className="text-h2 text-foreground">Workbook fields</h3>
            <div className="mt-4 flex flex-col gap-5">
              ...
              {preview.unsupportedSourceWarnings.length > 0 && (
                <div className="rounded-md border border-warning/40 bg-urgency-soon p-3 text-sm text-warning">
                  <h4 className="font-semibold">Source values not used</h4>
          ...
          <section className="min-w-0 lg:border-l lg:border-border lg:pl-8" aria-labelledby="import-mapping-title">
            <h3 id="import-mapping-title" className="text-h2 text-foreground">Mapping</h3>
            <div className="mt-4 flex flex-col gap-5">
              <div>
              ...
              <div>
                ...
                <p className="mt-1 break-words text-sm text-foreground">{preview.requestedAmountOrder.join(" → ")}</p>
              </div>
              <div>
                <h4 className="text-label text-muted-foreground">Awarded amount selection</h4>
                <p className="mt-1 break-words text-sm text-foreground">{preview.awardedAmountOrder.join(" → ")}</p>
              </div>
              <p className="border-t border-border pt-3 text-sm leading-5 text-muted-foreground">
                Excel dates and strict YYYY-MM-DD text use the workbook date system. Bare numeric years, formulas, and invalid dates are excluded rather than guessed. Populated amount values that do not fit the selected fields remain in each row&apos;s labeled source-values note.
              </p>
            </div>
          </section>
        </div>
      ...
      <section className="rounded-lg border border-border bg-card p-5 sm:p-6" aria-labelledby="import-rows-title">
        <div className="flex flex-col gap-1 sm:flex-row sm:items-baseline sm:justify-between">
          <div>
            <h3 id="import-rows-title" className="text-h2 text-foreground">Row decisions</h3>
            <p className="mt-1 text-sm text-muted-foreground">Review each source row, its mapped values, and its decision.</p>
          </div>
          <p className="text-sm text-muted-foreground">Structural rows are counted above and skipped; they are not invalid grants.</p>
        </div>
        {preview.rows.length > 0 ? (
          ...
          <p className="mt-5 border-t border-border pt-4 text-sm text-muted-foreground">No candidate rows were found in this workbook.</p>
        )}
      </section>

      <section className="rounded-lg border border-border bg-card p-5 sm:p-6" aria-labelledby="import-confirm-title">
        <h3 id="import-confirm-title" className="text-h2 text-foreground">Confirm import</h3>
        <p className="mt-1 max-w-3xl text-sm leading-5 text-muted-foreground">
          Confirming uploads this workbook again. The server re-checks access, mappings, funder matches, duplicate decisions, and row validity before creating records.
        </p>
        <div className="mt-4 rounded-md border border-warning/40 bg-urgency-soon p-3 text-sm leading-5 text-warning" role="alert">
          The same workbook can create additional grants if imported again. This confirmation applies to this import only.
        </div>
        ...
        <form className="mt-6 flex flex-col gap-5" onSubmit={onConfirm}>
          ...
          {preview.counts.valid === 0 && (
            <p className="text-sm text-destructive" role="status">No valid grants remain. Fix the workbook and analyze it again.</p>
          )}
          <div>
            <Button type="submit" disabled={!canConfirm} aria-disabled={!canConfirm}>
              {isConfirming ? "Importing…" : `Import ${preview.counts.valid} grant${preview.counts.valid === 1 ? "" : "s"}`}
            </Button>
          </div>
        </form>
      </section>
      ...
      {completion ? (
        ...
        <>
          <header className="max-w-3xl">
            <h1 className="text-title text-foreground">Import portfolio</h1>
            <p className="mt-2 text-base leading-6 text-muted-foreground">
              Choose an .xlsx workbook, review the rows, and confirm when ready. Choosing or analyzing a file does not change your portfolio.
            </p>
          </header>

          <section className="mt-8 border-y border-border py-5 sm:py-6" aria-labelledby="import-upload-title">
            <div className="flex flex-col gap-1">
              <h2 id="import-upload-title" className="text-h2 text-foreground">Choose your workbook</h2>
              <p className="text-sm leading-5 text-muted-foreground">Use an .xlsx workbook with the supported GrantFlow columns. Files must be 5 MiB or smaller.</p>
            </div>
            <form className="mt-5 flex flex-col gap-5" onSubmit={handleAnalyze}>
              <div>
                ...
                />
                <p className="mt-2 text-sm text-muted-foreground" id="portfolio-import-file-help">This file is used for this import only and is not saved as import history.</p>
