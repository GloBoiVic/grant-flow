...
export function GrantsPage({ grants, funders, selectedGrant, tags, createOpen, listQuery }: GrantsPageProps): React.ReactNode {
  ...
  return <div className="mx-auto w-full max-w-6xl px-4 py-7 sm:px-6 lg:px-8">
    <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><div><h1 className="text-title">Grants</h1><p className="mt-1 text-sm text-muted-foreground">Track opportunities, deadlines, and funding decisions.</p></div><div className="flex flex-wrap items-center gap-2"><Button asChild variant="outline"><a href="/export/portfolio" aria-describedby="portfolio-export-description">Export portfolio</a></Button><span id="portfolio-export-description" className="sr-only">Download the current non-deleted Grant portfolio and related Funder information as a CSV file.</span><Button onClick={openCreate} disabled={funders.length === 0} title={funders.length === 0 ? "Add a funder before creating a grant" : undefined}><Plus aria-hidden="true" /> Add grant</Button></div></div>
    ...
    {selectedGrant && <GrantDetailSheet key={selectedGrant.updatedAt} grant={selectedGrant} funders={funders} tags={tags} open onClose={closeSheet} />}
  </div>;
}
