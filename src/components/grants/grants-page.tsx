...
export function GrantsPage({ grants, funders, selectedGrant, tags, createOpen, listQuery }: GrantsPageProps): React.ReactNode {
  ...
  return <div className="mx-auto w-full max-w-7xl px-4 py-7 sm:px-6 lg:px-8">
    <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between"><h1 className="text-title">Grants</h1><div className="flex flex-wrap items-center gap-2"><Button asChild variant="outline"><a href="/export/portfolio" aria-describedby="portfolio-export-description">Export portfolio</a></Button><span id="portfolio-export-description" className="sr-only">Download the current non-deleted Grant portfolio and related Funder information as a CSV file.</span><Button onClick={openCreate} disabled={funders.length === 0} title={funders.length === 0 ? "Add a funder before creating a grant" : undefined}>Add grant</Button></div></div>
    {funders.length === 0 && <p className="mt-4 rounded-md border border-border bg-card px-3 py-2 text-sm text-muted-foreground">Add a <a className="font-medium text-primary underline-offset-4 hover:underline focus-visible:outline-2 focus-visible:outline-ring" href="/funders">funder</a> before creating a grant.</p>}
    <section className="mt-5" aria-label="Grant list filters">
      ...
      {filtersOpen && <div id="grant-filter-options" className="mt-2 flex flex-wrap gap-5 rounded-md border border-border bg-popover p-3" role="region" aria-label="Filter options">
    ...
    {selectedGrant && <GrantDetailSheet key={selectedGrant.updatedAt} grant={selectedGrant} funders={funders} open onClose={closeSheet} />}
  </div>;
}
