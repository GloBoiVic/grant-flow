...
interface GrantsPageProps { grants: GrantListDto; funders: FunderDto[]; selectedGrant: GrantDetailDto | null; createOpen: boolean; cursor: string | null; }

const statusClass: Record<string, string> = {
  Research: "bg-status-to-apply text-status-to-apply-fg", Qualified: "bg-status-to-apply text-status-to-apply-fg",
  Planning: "bg-status-in-progress text-status-in-progress-fg", Writing: "bg-status-in-progress text-status-in-progress-fg", "Internal Review": "bg-status-in-progress text-status-in-progress-fg",
  Submitted: "bg-status-submitted text-status-submitted-fg", Pending: "bg-status-submitted text-status-submitted-fg",
  Awarded: "bg-status-approved text-status-approved-fg", Reporting: "bg-status-approved text-status-approved-fg", Declined: "bg-status-declined text-status-declined-fg", Closed: "bg-status-to-apply text-status-to-apply-fg",
};

function money(value: string | null, currency: string): string { return value ? new Intl.NumberFormat("en-US", { style: "currency", currency }).format(Number(value)) : "—"; }
function date(value: string | null): string { return value ? new Intl.DateTimeFormat("en-US", { month: "short", day: "numeric", year: "numeric", timeZone: "UTC" }).format(new Date(`${value}T00:00:00Z`)) : "—"; }

export function GrantsPage({ grants, funders, selectedGrant, createOpen, cursor }: GrantsPageProps): React.ReactNode {
  ...
  return <div className="mx-auto w-full max-w-6xl px-4 py-7 sm:px-6 lg:px-8">
    ...
    {selectedGrant && <GrantDetailSheet grant={selectedGrant} funders={funders} open onClose={closeSheet} />}
  </div>;
}
