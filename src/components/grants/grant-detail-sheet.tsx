    ...
    <Sheet open={open && !isEditing} onOpenChange={(next) => { if (!next && !isEditing) onClose(); }}>
      <SheetContent>
        ...
        <div className="min-h-0 flex-1 overflow-y-auto px-4 pb-5">
          <div className="flex flex-wrap gap-2 border-b border-border pb-4"><Button variant="outline" size="sm" onClick={() => setIsEditing(true)}>Edit</Button><Link href={`/grants/${encodeURIComponent(grant.id)}`} className="inline-flex h-8 items-center rounded-md border bg-background px-3 text-sm font-medium shadow-xs hover:bg-accent hover:text-accent-foreground focus-visible:outline-2 focus-visible:outline-ring">Open full grant</Link><div className="flex items-center gap-2"><label htmlFor="grant-detail-status" className="sr-only">Change grant status</label><select id="grant-detail-status" value={status} onChange={(event) => setStatus(event.target.value as GrantDetailDto["status"])} className="h-8 rounded-md border border-input bg-background px-2 text-sm outline-none focus-visible:ring-2 focus-visible:ring-ring/50">{Object.values(GrantStatus).map((value) => <option key={value}>{value}</option>)}</select><Button size="sm" onClick={saveStatus} disabled={isSaving}>{isSaving ? "Saving…" : "Change status"}</Button></div></div>
